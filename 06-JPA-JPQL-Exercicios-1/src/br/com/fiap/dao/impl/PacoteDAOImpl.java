...
public class PacoteDAOImpl extends GenericDAOImpl<Pacote,Integer> implements PacoteDAO{
	...
	@Override
	public Long pctsPorData(Calendar ini, Calendar fim) {
		
		return em.createQuery("select count(p) from Pacote p where p.dataSaida between :ini and :fim",
				Long.class).
	...
	public List<Pacote> maiorPreco(){
		Float maior = em.createQuery("select max(p.preco)from Pacote p",Float.class).getSingleResult();
		return em.createQuery("from Pacote p where p.preco >= :maj",Pacote.class).setParameter("maj", maior).getResultList();
	}
